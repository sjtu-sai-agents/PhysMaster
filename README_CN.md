...
| **Supervisor** | 读取树上下文（HCC），选择下一个子任务，决定 draft 还是 revise |
| **Theoretician** | 求解子任务 &mdash; 可调用 Python、技能包、网页搜索和先验知识库 |
| **Critic** | 对解答打分（0&ndash;1）并做出判决：`complete` / `to_revise` / `to_redraft` |
| **Promoter** | 将原始经验（L1）压缩为树上下文中的可复用知识（L2） |
...
| **提炼** | Promoter 将原始经验压缩为简洁的知识摘要 |
| **反向传播** | 奖励向上流动；高奖励节点（&gt;0.8）强化祖先节点 |
...
```
 ┌──────────────────────────────────────────────────────────────────┐
 │  L3  Wisdom          跨任务         → FAISS 先验索引             │
 │  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─  │
 │  L2  Knowledge       搜索树内       → node.knowledge             │
 │  ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─ ─  │
 │  L1  Experience      单个节点       → node.experience             │
 └──────────────────────────────────────────────────────────────────┘
```

- **L1 &mdash; Experience** &nbsp; 原始 Theoretician 输出：完整推理、工具调用、代码。每节点存储。
- **L2 &mdash; Knowledge** &nbsp; 评估后由 Promoter 压缩。通过树上下文传播，使后代节点看到祖先的经验。当节点得分 &gt;0.8 时，其 L2 在反向传播时标记到祖先（*认知强化*）。
- **L3 &mdash; Wisdom** &nbsp; 整个任务完成后蒸馏。写回 FAISS 先验索引，使**未来任务**可以检索到。
...
landau:
  prior_enabled: true
  wisdom_save_enabled: true   # 可选：持久化 L3 wisdom
```
